...
describe("Fastify", () => {
  ...
  test("the former info path is no longer served", async () => {
    process.env.HIREFIRE_TOKEN = "SOME_TOKEN"
    HireFire.configuration.dyno("worker", () => 5)
    // No route is registered for it, so Fastify 404s — the middleware no longer
    // intercepts and serves worker JSON.
